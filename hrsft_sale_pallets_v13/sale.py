...
class product_product(models.Model):  
    ...
    qty_min_par_pallet = fields.Integer(string='Quantité minimale')
    type_pallet = fields.Selection([('pallet1','Pallet1'),('type2','Type2'),('type3','Type3')],string='Type de Pallet')
    pallet = fields.Many2one('product.product',string='Pallet',readonly=False, domain="[('is_pallet', '=', True)]")
    #category_id = fields.Char('pallet category id ',compute='get_categorie_pallet_id')
    is_pallet = fields.Boolean(string='Pallet', default=False)
    ...
    def get_calc_pallets(self,product_qty):
        ...
        if self.qty_par_pallet > 0:
            if (product_qty % self.qty_par_pallet) >= self.qty_min_par_pallet:
                pallets = int(product_qty  / self.qty_par_pallet) + 1
            else:
                pallets = int(product_qty  / self.qty_par_pallet)
